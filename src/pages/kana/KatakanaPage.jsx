...
import { KATA } from '../../data/kanaData';

const groups = [
  { key: 'basic',   label: 'Basic',        sub: 'ア → ン  ·  46 chars' },
  { key: 'dakuten', label: 'Voiced',        sub: 'ガ → ポ  ·  25 chars' },
  { key: 'combo',   label: 'Combinations', sub: 'キャ → ウォ  ·  36 chars' },
];
        ...
        <div key={row.r} className="mb-5">
          ...
          <div className="flex flex-wrap gap-2">
            {row.chars.map(ch => (
              ...
              >
                {(view === 'all' || view === 'jp') && (
                  <span className="block text-2xl font-bold text-amber-500 mb-0.5 leading-none" style={{fontFamily:'Noto Sans JP,sans-serif'}}>{ch.c}</span>
                )}
                {(view === 'all' || view === 'rom') && (
      ...
      {popup && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setPopup(null)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl p-8 text-center w-full sm:max-w-[260px] shadow-xl" onClick={e => e.stopPropagation()}>
            <span className="block text-[5.5rem] font-bold text-amber-500 mb-3 leading-none" style={{fontFamily:'Noto Sans JP,sans-serif'}}>{popup.c}</span>
            <div className="text-2xl font-black text-slate-800 mb-1">{popup.r}</div>
            {popup.h && (
              <div className="text-base text-pink-500 mb-1" style={{fontFamily:'Noto Sans JP,sans-serif'}}>
                Hiragana: {popup.h}
              </div>
            )}
            <button
              onClick={() => setPopup(null)}
              className="mt-4 px-5 py-2 bg-slate-100 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-200 transition cursor-pointer border-none"
            >
              Close
            </button>
          </div>
